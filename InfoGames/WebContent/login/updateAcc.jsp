...
ID(최대 15자):
<input type="text" name="id" maxlength="15" onkeydown="inputIdChk()" value="<%=session.getAttribute("id") %>" placeholder="영문, 숫자만 가능"/>
<input type="button" onclick="openChkId()" value="중복확인"><br><br>
<input type="hidden" name="idDuplication" value="idUncheck">
비밀번호:
<input type="password" name="pw" value="<%=session.getAttribute("pw")%>" maxlength="25"/><br><br>
비밀번호확인:
<input type="password" name="chkPw" value="<%=session.getAttribute("pw")%>" maxlength="25"/><br><br>
Email:
<input type="text" name="email" value="<%=session.getAttribute("email") %>" maxlength="40" placeholder="ex)asd123@naver.com"/><br><br>
닉네임(최대 10자):
<input type="text" name="nickName" maxlength="10" value="<%=session.getAttribute("nickName")%>" onkeydown="inputNickNameChk()"/>
