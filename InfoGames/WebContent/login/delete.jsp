...
<head>
<meta charset="UTF-8">
<title><%=session.getAttribute("id")%>(<%=session.getAttribute("nickName")%>) 계정 삭제하기</title>
</head>
...
<body>
	<h3><%=session.getAttribute("id")%>(<%=session.getAttribute("nickName")%>) 계정 삭제하기</h3>
	<br>
	<form name="frm" action="deleteOk.do" method="post" onsubmit="return chkSubmit()">
		비밀번호:
